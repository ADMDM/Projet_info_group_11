...
# Tache 1 : Traitement des données
# Création des groupes de données en utilisant la méthode "groupby"
# Groupe de données : Irradiance et précipitations journalières cumulées 
daily_weather = weather.groupby(by='Day')[['Irradiance', 'Rain']].sum()
# Groupe de données : Température, Vitesse du vent, Humidité relative et Déficit de saturation journaliers moyens
daily_weather1 = weather.groupby(
    by='Day')[['Temp', 'Wind', 'Rel_hum', 'SD']].mean()
#Groupe de données : H45T et H45E journaliers cumulées (humidité volumique dans le sol à 45 cm en foret eclaircie et temoin )
daily_soil = soil.groupby(by='Day')[['H45T', 'H45E']].sum()
# Groupe de données : Rain_E et Rain_T journaliers cumulées 
daily_soil1 = soil.groupby(by='Day')[['Rain_E', 'Rain_T']].sum()

#          1.1.5 Groupe de données : E153, E159, E161, T13, T21 et T22 journaliers moyennes 
daily_sfd = sfd.groupby(
    by='Day')[['E153', 'E159', 'E161', 'T13', 'T21', 'T22']].mean()

#           1.1.6 Fusion des groupes de données en un seul dataframe
...
#    2. Conversion de l'irradiance en kWh/m^2
data['Irradiance'] = data['Irradiance'] / 360


#   3.  Changement d'index de data : nouvel index = Date complete format datetime 
data['Date'] = pd.date_range('1999-1-1', periods=1096).strftime('%Y-%m-%d')
data.set_index('Date', inplace=True)


#   4. Enregistrement du dataframe data dans un fichier CSV
data.to_csv('data_LBIR1271.csv')

#                   Analyse exploratoire des donnés 

#       1.  Calcul des statistiques principales des variables
...
#       2. Chargement du dataframe data depuis le fichier CSV
...
#   - Figure 2: Irrandiance, Rain,Temp,Wind,Rel_hum et SD
...
# - Figure 3: E153,E159,E161,T13,T21 ET T22
...
variables = ["Temp", "Rain", "Wind", "Irradiance",
             "Rel_hum", "SD", "Rain_E", "H45E","H45T","E153","E159","E161","T13","T21","T22"]
